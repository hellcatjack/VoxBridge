...
`/listen` 的 Live Audio 区域会显示当前设备正在朗读的译文，而不是后端最新生成的译文。后端把已经稳定发布到共享 HLS 的语音维护为最多 256 条带墙钟时间的字幕提示；浏览器根据自己的 HLS live edge 延迟选择当前提示，因此不同播放速度或网络缓存的设备仍能各自对齐。句间停顿时保留并弱化上一句，不清空或闪烁。字幕元数据轮询只在页面前台运行，失败或锁屏暂停不会阻塞原生 HLS 音频。
