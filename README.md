...
每台监听设备可在 `/listen` 独立选择 `0.75x`、`1.0x`、`1.25x`、`1.5x` 或 `2.0x` 朗读速度。选择保存在该浏览器中，播放期间修改会立即生效，Stop、重连和刷新不会重置；它不影响其他设备，也不会让后端重复合成共享 WAV。
...
可见字幕和译文继续实时更新；朗读采用更严格的后端稳定门，默认要求最新句子 revision 连续 3.0 秒不再变化后才发布。若同一句在窗口内收到更高 revision，旧译文不会进入朗读队列。正常 Stop 会在最终 ASR 与翻译完成后放行最后的 ready 版本，因此该保护不会让会议最后一句永久滞留。
