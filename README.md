...
多个翻译 worker 即使乱序完成，也会按源句顺序发布。无论同时连接多少台设备，每条译文只由一个 Kokoro worker 合成一次，也只进入一个 FFmpeg 编码器；设备读取同一套 HLS 分片。设备 Stop 仅删除自己的租约，最后一个租约离开或超过 90 秒未刷新后，后端关闭共享编码器并清理临时分片，但之后产生的稳定译文仍可进入有界待播池。页面 `playback speed` 只调整设备消费已编码 HLS 的速度，不能删除已经编码进时间线的静音。低速播放落后 live edge 达到 12 秒时会临时以 `1.2x` 追赶，回到 5 秒内后恢复用户选择；页面进入锁屏/后台时最低使用 `1.0x`。该低速追赶只调整倍速，不会 seek 或丢弃译文语音；独立的历史静音压缩仅在后续译文已经缓冲时跳过多余的已编码静音，并保留自然停顿。

每台监听设备可在 `/listen` 独立选择默认的 `Auto`，或 `0.8x` 至 `1.4x`（每档 `0.1x`）的固定朗读速度。选择保存在该浏览器中，播放期间修改会立即生效，Stop、重连和刷新不会重置；旧版页面保存的不受支持值会安全回退到 `Auto`。该设置不影响其他设备，也不会让后端重复合成共享 WAV。

共享 HLS 在服务器端保持严格 FIFO。Kokoro 输出的 PCM 后固定追加 `300ms` 句号停顿，再写入同一条连续音频时间线；停顿不分析中英文文本。译文队列和 FFmpeg 前的 PCM 队列都有上限，编码变慢时通过背压限制内存增长，而不是按监听设备复制音频或启动额外合成任务。`GET /api/tts/live/status` 的 `pending_audio_ms` 显示已经合成、仍在等待写入实时 HLS 时间线的音频时长；`translated_audio_backlog_ms` 进一步包括已经成功翻译、但尚未写入 HLS 的去重句子。已经预合成的句子使用精确 PCM 时长，其余句子根据该语言近期 Kokoro 的实际字符/时长比估算。

`/listen` 的 Live Audio 区域会显示当前设备正在朗读的译文，而不是后端最新生成的译文。它显示的等待音频时长等于本设备到 HLS live edge 的距离，加上服务器的 `translated_audio_backlog_ms`；追赶时间再按当前实际播放倍速折算。Auto 按端到端总等待时间选择 `<10s 1.0x`、`<30s 1.2x`、`<40s 1.4x`、`>=40s 1.5x`；目标倍速高于 `1.0x` 时仍须等本设备至少积累 4 秒 HLS 缓冲，缓冲降到 2 秒时暂时恢复 `1.0x`。后续译文已安全缓冲时，页面可以把较长的历史静音压缩为至少 500ms 的自然停顿；seek 只有在媒体重新进入 `playing` 后才算完成，800ms 未恢复会自动重试。后端按实际写入 FFmpeg 的 PCM 媒体样本建立最多 256 条绝对时间字幕提示，计入 AAC 编码帧延迟，并通过与语言无关的波形活动检测排除 Kokoro 首尾静音。Safari 使用原生 `getStartDate() + currentTime` 定位该设备正在播放的节目时间，因此不同倍速、网络缓存或播放列表更新进度不会把服务器的新字幕提前显示。句间停顿时保留并弱化上一句，不清空或闪烁。字幕元数据轮询只在页面前台运行，失败或锁屏暂停不会阻塞原生 HLS 音频。
